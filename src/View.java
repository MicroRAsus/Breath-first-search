...
class View extends JPanel implements MouseListener {
	Viz viz;
	ArrayList<byte[]> bytePath;
	Graphics graphics;
	int size;
	private byte[] state;
	private int clickCounter;

	View(Viz v, ArrayList<byte[]> bytePath) throws IOException
	{
		viz = v;
	...
	{
		if(this.clickCounter < this.bytePath.size()) {
			this.state = bytePath.get(clickCounter);
			this.clickCounter++;
		}
